...
#define SSTAR_WLAN_CATEGORY_PUBLIC 4 //IEEE 802.11 public action frames
...
u8 *Sstar_get_p2p_attr(u8 *p2p_ie, u32 p2p_ielen, u8 target_attr_id ,u8 *buf_attr, u32 *len_attr)
{
	u8 *attr_ptr = NULL;
...
u8 *Sstar_get_p2p_attr_content(u8 *p2p_ie, u32 p2p_ielen, u8 target_attr_id ,u8 *buf_content, u32 *len_content)
{
	u8 *attr_ptr;
	u32 attr_len;
	...
	attr_ptr = Sstar_get_p2p_attr(p2p_ie, p2p_ielen, target_attr_id, NULL, &attr_len);
...
u8 P2P_change_negotation_intent(u8 *pframe, u32 len ,int intent)
{
	...
	if ( !p2p_ie ) 	{
		...
		pAttr_content = Sstar_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GO_INTENT , &attr_content, &attr_contentlen);
...
static bool  ieee80211_parase_p2p_action_attrs(u8* ie_start,ssize_t ie_len,struct Sstar_p2p_message *p2p_msg)
{
	u8* p2p_ie = NULL;
	u8* attr_start = NULL;
	ssize_t attr_len = 0;
	...
	while(pos<end)
	{
		...
		if(p2p_msg->capability == NULL)
			p2p_msg->capability = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_CAPABILITY);
		if(p2p_msg->go_intent == NULL)
			p2p_msg->go_intent = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_GROUP_OWNER_INTENT);
		if(p2p_msg->status == NULL)
			p2p_msg->status = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_STATUS);
		if(p2p_msg->operating_channel == NULL)
			p2p_msg->operating_channel = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_OPERATING_CHANNEL);
		if(p2p_msg->channel_list == NULL)
			p2p_msg->channel_list = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_CHANNEL_LIST);
		if(p2p_msg->group_bssid == NULL)
			p2p_msg->group_bssid = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_GROUP_BSSID);
		if(p2p_msg->group_info == NULL)
			p2p_msg->group_info = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_GROUP_INFO);
		if(p2p_msg->group_id == NULL)
			p2p_msg->group_id = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_GROUP_ID);
		if(p2p_msg->device_id == NULL)
			p2p_msg->device_id = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_DEVICE_ID);
		if(p2p_msg->manageability == NULL)
			p2p_msg->manageability = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_MANAGEABILITY);
		if(p2p_msg->intended_addr == NULL)
			p2p_msg->intended_addr = Sstar_ieee80211_find_p2p_attr(attr_start,attr_len,SSTAR_P2P_ATTR_INTENDED_INTERFACE_ADDR);
		p2p_find = 1;
...
//		printk(KERN_ERR "%s:len err(%d),ie_len(%d)\n",__func__,SSTAR_WPA_GET_LE16((const u8*)(&channel_list[1])),ie_len);
	...
	while(pos+2<end)
	{
		...
		for(index=0;index<number_channel;index++){
			if(pos[index]<=14){
				Sstar_printk_p2p("change channel(%d) to channel(%d)\n",pos[index],channel_num);
				pos[index] = channel_num;
			}
		}
...
void Sstar_parase_p2p_scan_resp(struct Sstar_vif *priv,struct sk_buff *skb)
{
	...
	*/
	...
	Sstar_printk_p2p("%s:go_bssid_set(%d),receive_go_resp(%d)\n",__func__,atomic_read(&hw_priv->go_bssid_set),
	...
	Sstar_printk_p2p("%s:go_bssid(%pM),bssid(%pM)\n",__func__,hw_priv->go_bssid,mgmt->bssid);
	if(!memcmp(hw_priv->go_bssid,mgmt->bssid,6)){
		atomic_set(&hw_priv->receive_go_resp,1);
	}
}
bool Sstar_check_channel_combination(struct Sstar_common *hw_priv,struct Sstar_vif *ignore_priv)
{
	BUG_ON(!ignore_priv);
	...
	*/
	{
		...
		u8 persistent_bit = SSTAR_P2P_GROUP_CAPAB_PERSISTENT_GROUP | SSTAR_P2P_GROUP_CAPAB_PERSISTENT_RECONN;
...
bool Sstar_parase_p2p_mgmt_frame(struct Sstar_vif *priv,struct sk_buff *skb,bool tx)
{
	...
	if(probe_req){
		...
		ie_len = skb->len - offsetof(struct Sstar_ieee80211_mgmt, u.probe_req.variable);
	}
	else if(probe_resp){
		ie = mgmt->u.probe_resp.variable;
		ie_len = skb->len - offsetof(struct Sstar_ieee80211_mgmt, u.probe_resp.variable);
	}
...
//	#define SSTAR_CHANGE_LOCAL_REMOUT_ROLE
	...
	if (len < IEEE80211_MIN_ACTION_SIZE){
		Sstar_printk_p2p("%s:len(%d) is err\n",__func__,len);
		return false;
	}

	if(mgmt->u.action.category != WLAN_CATEGORY_PUBLIC){
		Sstar_printk_p2p("%s:category(%d) is err\n",__func__,mgmt->u.action.category);
		return false;
	}

	p2p_data = (u8*)(&mgmt->u.action.u.vs_public_action);
	p2p_data_len = len - offsetof(struct Sstar_ieee80211_mgmt, u.action.category);

	if((p2p_check_offs = ieee80211_p2p_action_check(p2p_data,p2p_data_len))==-1){
		Sstar_printk_err("%s:p2p_check_offs is err\n",__func__);
		return false;
	}
	...
	*/
	while(p2p_msg.go_intent){
		...
		if(tx){
			*go_intent_pos = SSTAR_GO_INTEND; //we send intend and tie breaker 0
		}else {
	...
	while(p2p_msg.operating_channel){
		...
		Sstar_printk_p2p("%s:chan_mode(%x),operating_channel_combination(%d)\n",__func__,(int)combination,
		...
		Sstar_printk_p2p("%s:operating_channel_num(%d),local_channel(%d),action(%d),tx(%d)\n",__func__,
		...
		*/
		...
		if((p2p_data[0] != SSTAR_P2P_INVITATION_REQ)&&(p2p_data[0] != SSTAR_P2P_INVITATION_RESP)){
			atomic_set(&hw_priv->p2p_oper_channel,0);
			if(((p2p_data[0] != SSTAR_P2P_GO_NEG_REQ)&&(p2p_data[0] != SSTAR_P2P_GO_NEG_RESP))||(combination == false))
				break;
			else{
				Sstar_printk_p2p("%s:change_channel_and_list,local_channel(%d)\n",__func__,local_channel);
		...
		*/
		if(p2p_data[0] == SSTAR_P2P_INVITATION_REQ)
			atomic_set(&hw_priv->p2p_oper_channel,0);
		Sstar_printk_p2p("%s:tx(%d),action(%d),operating_channel_num(%d),p2p_oper_channel(%d)\n",
		...
		if(((p2p_data[0] == SSTAR_P2P_INVITATION_REQ)&&(combination == true))||
		   ((p2p_data[0] == SSTAR_P2P_INVITATION_RESP)&&(atomic_read(&hw_priv->operating_channel_combination))&&(combination == true))){
			goto change_channel_and_list;
		}
...
set_oper_channel:
	...
	while(((p2p_data[0] == SSTAR_P2P_INVITATION_REQ)||(p2p_data[0] == SSTAR_P2P_INVITATION_RESP))&&(p2p_msg.group_bssid))
	{
		if(SSTAR_WPA_GET_LE16((const u8*)(p2p_msg.group_bssid+1)) != 6){
			Sstar_printk_err("%s:group_bssid is err(%d)\n",__func__,SSTAR_WPA_GET_LE16((const u8*)(p2p_msg.group_bssid+1)));
			break;
		}
	...
	while(tx == false)
	{
		...
		*/
		if((p2p_data[0] != SSTAR_P2P_GO_NEG_REQ)&&(p2p_data[0] != SSTAR_P2P_GO_NEG_RESP))
			break;
		if(!p2p_msg.intended_addr)
			break;
		if(SSTAR_WPA_GET_LE16((const u8*)(p2p_msg.intended_addr+1)) != 6){
			Sstar_printk_err("%s:intended_addr is err(%d)\n",__func__,SSTAR_WPA_GET_LE16((const u8*)(p2p_msg.intended_addr+1)));
			break;
		}
		
		atomic_set(&hw_priv->go_bssid_set,1);
		memcpy(hw_priv->go_bssid,p2p_msg.intended_addr+3,6);
		Sstar_printk_err("%s:intended_addr(%pM),own_addr(%pM)\n",__func__,p2p_msg.intended_addr+3,priv->vif->addr);
		break;
	}
