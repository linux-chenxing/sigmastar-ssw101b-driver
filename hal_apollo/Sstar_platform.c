...
#endif
#if ((SSTAR_WIFI_PLATFORM == PLATFORM_AMLOGIC_S805) || (SSTAR_WIFI_PLATFORM == PLATFORM_AMLOGIC_905))
...
static int Sstar_platform_power_ctrl(const struct Sstar_platform_data *pdata,bool enabled)
{
	int ret = 0; 
#ifndef USB_BUS	
	#if (SSTAR_WIFI_PLATFORM == PLATFORM_XUNWEI) ||(SSTAR_WIFI_PLATFORM == PLATFORM_FRIENDLY)
	{
	...
	enabled = 1;
		...
		if (gpio_request(pdata->power_gpio, "WIFI_POWERON")!=0) {
			Sstar_printk_platform("[sigmastar] ERROR:Cannot request WIFI_POWERON\n");
		} else {
			gpio_direction_output(pdata->power_gpio, 1);/* WLAN_CHIP_PWD */
			gpio_set_value(pdata->power_gpio, enabled);
			mdelay(1);
			gpio_free(pdata->power_gpio);
			Sstar_printk_platform("[sigmastar] + %s :EXYNOS4_GPC1(0) wlan powerwew %s\n",__func__, enabled?"on":"off");
		}
	}
...
static int Sstar_platform_insert_crtl(const struct Sstar_platform_data *pdata,bool enabled)
{
	int ret = 0;
#ifdef SDIO_BUS
	#if (SSTAR_WIFI_PLATFORM == PLATFORM_XUNWEI)
...
int Sstar_plat_request_gpio_irq(const struct Sstar_platform_data *pdata,struct sbus_priv *self,int * Sstar_bgf_irq)
{
	int bgf_irq =0;
	int ret =0;
	...
	/* Request the IRQ */
	ret =  request_threaded_irq(bgf_irq, Sstar_gpio_hardirq,
					Sstar_gpio_irq,
...
#define IRQ_THREAD_REQ
	...
	/* Request the IRQ */
	ret =  request_threaded_irq(bgf_irq, Sstar_gpio_irq,
				    NULL,
	...
	if (gpio_is_valid(pdata->irq_gpio)) {
		...
		if (ret) {
			Sstar_printk_platform("BGF_EINT gpio request fail, ret = %d\n", ret);
		}
	} else {
...
void Sstar_plat_free_gpio_irq(const struct Sstar_platform_data *pdata,struct sbus_priv *self,int Sstar_bgf_irq)
{
#if(SSTAR_WIFI_PLATFORM == PLATFORM_AMLOGIC_905)
