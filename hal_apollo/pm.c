...
int _atbm_wow_suspend(struct ieee80211_hw *hw, struct cfg80211_wowlan *wowlan)
{
	...
	if (atbm_bh_suspend(hw_priv)) {
		atbm_printk_err("%s: atbm_bh_suspend failed\n",
				__func__);
		atbm_wow_resume(hw);
		return -EBUSY;
	}
	...
	if (ret) {
		atbm_printk_err( "%s: PM request failed: %d. WoW is disabled.\n",
			__func__, ret);
		atbm_wow_resume(hw);
		return -EBUSY;
	}
	...
	if (atomic_read(&hw_priv->bh_rx)) {
		atbm_printk_err("%s: incoming event present - resume\n",
				__func__);
		atbm_wow_resume(hw);
		return -EAGAIN;
	}
...
int atbm_wow_suspend(struct ieee80211_hw *hw, struct cfg80211_wowlan *wowlan)
{
	...
	*/
	...
	atbm_for_each_vif(hw_priv, priv, i) {
		...
		&& (priv->join_status == ATBM_APOLLO_JOIN_STATUS_STA)) {
			wsm_lock_tx(hw_priv);
			atbm_printk_pm( "[%s] line:%d unjoin_work\n", __func__, __LINE__);
			mutex_unlock(&hw_priv->conf_mutex);
			up(&hw_priv->scan.lock);
			...
			atbm_printk_pm("[%s] line:%d should stop ap\n", __func__, __LINE__);
		}
	}
	mutex_unlock(&hw_priv->conf_mutex);
	up(&hw_priv->scan.lock);
