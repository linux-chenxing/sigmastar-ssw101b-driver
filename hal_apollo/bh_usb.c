...
int atbm_rx_bh_cb(struct atbm_common *hw_priv,struct sk_buff *skb)
{
	...
	if((hw_priv->wsm_caps.firmwareReady==0)
		...
		printk("rx wsm_seq err1 (rxseq=%d need_rxseq=%d) len(%d) wsm_id %x firmwareReady %d\n",wsm_seq,hw_priv->wsm_rx_seq,skb->len,wsm_id,hw_priv->wsm_caps.firmwareReady);
		//BUG_ON(1);
		atbm_usb_bh_halt(hw_priv);
		goto __free;
	}
	else if (WARN_ON(wsm_seq != hw_priv->wsm_rx_seq)) {
		printk(KERN_ERR "rx wsm_seq error wsm_seq[%d] wsm_rx_seq[%d] skb->len[%d] wsm_id[%x],wsm_len[%d]\n",
...
void atbm_rx_tasklet(unsigned long priv)
{
	...
	while ((skb = atbm_skb_dequeue(&hw_priv->rx_frame_queue)) != NULL) {
		...
		if(wsm_id == WSM_MULTI_RECEIVE_INDICATION_ID){
			atbm_rx_multi_rx(hw_priv,skb,atbm_rx_bh_cb);
			...
			hw_priv->wsm_rx_seq = (wsm_seq + 1) & 7;
			atbm_rx_multi_rx(hw_priv,skb,atbm_rx_single_channel_bh_cb);
		}
		else { 
