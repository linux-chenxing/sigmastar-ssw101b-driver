...
void atbm_parase_p2p_scan_resp(struct atbm_vif *priv,struct sk_buff *skb)
{
	...
	*/
	...
	atbm_printk_p2p("%s:go_bssid_set(%d),receive_go_resp(%d)\n",__func__,atomic_read(&hw_priv->go_bssid_set),
	...
	atbm_printk_p2p("%s:go_bssid(%pM),bssid(%pM)\n",__func__,hw_priv->go_bssid,mgmt->bssid);
...
//	#define ATBM_CHANGE_LOCAL_REMOUT_ROLE
	...
	if (len < IEEE80211_MIN_ACTION_SIZE){
		atbm_printk_p2p("%s:len(%d) is err\n",__func__,len);
		return false;
	}

	if(mgmt->u.action.category != WLAN_CATEGORY_PUBLIC){
		atbm_printk_p2p("%s:category(%d) is err\n",__func__,mgmt->u.action.category);
		return false;
	}
	...
	if((p2p_check_offs = ieee80211_p2p_action_check(p2p_data,p2p_data_len))==-1){
		atbm_printk_err("%s:p2p_check_offs is err\n",__func__);
		return false;
	}
