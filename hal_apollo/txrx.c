...
{	
	...
	if((priv->dhcp_retry_skb != NULL) &&(SDATA_IS_CONNECTTING(vif_to_sdata(priv->vif)))){
		...
		spin_unlock_bh(&priv->dhcp_retry_spinlock);
		printk(KERN_ERR"atbm_dhcp_retry_work __atbm_tx\n");
		local_bh_disable();
		__atbm_tx(priv->hw,skb);
		local_bh_enable();
		spin_lock_bh(&priv->dhcp_retry_spinlock);
		tx_dhcp_retry_cnt++;
		printk(KERN_ERR"restart dhcp_retry_work %d\n",tx_dhcp_retry_cnt);
...
#ifdef USB_BUS_BUG
	if(__cpu_to_le16(t->skb->len) < 1538)
		wsm->hdr.usb_len = 1538;//__cpu_to_le16(t->skb->len);
	else 		
		wsm->hdr.usb_len = __cpu_to_le16(t->skb->len);
	...
	if((assoc == false)||
		...
		*/
		while((t->tx_info->control.vif->type == NL80211_IFTYPE_STATION)&&
		   ...
		   if(cbss == NULL){
		   		printk(KERN_ERR "%s cbss is null\n",__func__);
		   		break;
		   }
			...
			struct wsm_tx *wsm)
{
	...
	*/
	...
	if(t->tx_info->flags&IEEE80211_TX_CTL_USE_MINRATE){
		wsm->htTxParameters |= WSM_HT_TX_USE_MINRATE;
	}
...
*/
	...
	*/
	...
	if((IEEE80211_TX_CTL_REQ_TX_STATUS |IEEE80211_TX_CTL_INJECTED)&t.tx_info->flags){
		...
		wsm->htTxParameters |=__cpu_to_le32(WSM_NEED_TX_CONFIRM);
		printk(KERN_DEBUG "%s:qos null data\n",__func__);
	} 
	...
	if ((arg->status == WSM_REQUEUE) &&
		...
		*/
		while(arg->status){
			if(atbm_queue_get_generation(arg->packetID) >= 2){
				printk(KERN_ERR "%s:eap retry to mamy(%x)\n",__func__,arg->packetID);
				break;
			}
			...
			printk(KERN_ERR "retry eap frame\n");
			atbm_priv_vif_list_read_unlock(&priv->vif_lock);
		...
		*/
		while((arg->status)&&(arg->status != 1)){
			...
			*/
			if(frame==NULL){
				printk(KERN_ERR "frame is null\n");
				atbm_priv_vif_list_read_unlock(&priv->vif_lock);
				BUG_ON(1);
				return ;
			}
			if(atbm_queue_get_generation(arg->packetID) >= 5){
				printk(KERN_ERR "%s:retry to many times(%x)\n",__func__,arg->packetID);
				break;
			}
			...
			printk(KERN_ERR "%s:retry[%d][%x],start[%ld],duration[%ld],now(%ld)\n",__func__,arg->status,
...
#ifdef CONFIG_WIRELESS_EXT
extern int atbm_best_ch_update_buff(const u8 *ssid, u8 ssid_len, u8 *mac_addr, u8 rssi, u8 channel);
#endif
int atbm_handle_best_channel_scan(struct wsm_rx *arg, struct sk_buff *skb)
{
	u8 *ies = NULL;
	size_t ies_len;
	const  u8 *ie,*ssid;
	u8 ssid_len, channel, rssi;
	u8 mac[ETH_ALEN];
	...
	//MAC
	memcpy(mac, frame->addr3, ETH_ALEN);

	//Channel
	channel = arg->channelNumber;

	//RSSI
	rssi = arg->rcpiRssi;
#ifdef CONFIG_WIRELESS_EXT
	atbm_best_ch_update_buff(ssid, ssid_len, mac, rssi, channel);
#endif
	return 0;
}

#ifdef ATBM_PRIVATE_IE
int atbm_handle_private_ie(struct atbm_vif *priv, struct wsm_rx *arg, struct sk_buff **skb_p)
{
	...
	extern unsigned char private_scan_sta;
	...
	u8 *ies=NULL;
	size_t ies_len;
	const u8 *priv_ie=NULL,*ie=NULL,*pdata=NULL;
	u8 len;
	...
	if((private_scan_sta == 1) && (found_pkt == 1)){
			...
			if(0 == memcmp(priv_ie_f->Oui, atbm_oui, 4)){
				...
				for(i=0;i<8;i++){
					...
					if((99 == filter_sta) || (0 == memcmp(filter_mac_g[i], frame->addr3, ETH_ALEN))){
						...
						//SSID
						ie = atbm_get_ie(ies, ies_len, ATBM_WLAN_EID_SSID);
						ds_ie = atbm_get_ie(ies, ies_len, ATBM_WLAN_EID_DS_PARAMS);
...
#ifdef SIGMASTAR_FILTER_MACADDR_ONLY
						if(priv_ie_f)
#endif
						{
							//User Data
...
#endif
extern int atbm_set_rssi_val(int val);
		  ...
		  struct sk_buff **skb_p)
{
	...
	if (unlikely(arg->status)) {
		if (arg->status == WSM_STATUS_MICFAILURE) {
			printk(KERN_ERR "[RX] MIC failure. ENCRYPTION [%d][%pM][%pM]\n",
...
#ifdef ATBM_PRIVATE_IE
	if(0 != atbm_handle_private_ie(priv, arg, skb_p)){
		goto drop;//AP mode, no need to process probe request pkt
	}
#endif

#ifdef CONFIG_WIRELESS_EXT	
	if((atbm_best_ch_scan_status_get() == 1) &&
		(ieee80211_is_beacon(frame->frame_control)||ieee80211_is_probe_resp(frame->frame_control))){
		//printk("Best channel scan----start\n");
		atbm_handle_best_channel_scan(arg, skb);
	}		
#endif
