...
ieee80211_associate(struct ieee80211_work *wk)
{
	...
	if (wk->assoc.tries > IEEE80211_ASSOC_MAX_TRIES) {
		...
		atbm_printk_err("%s:ieee80211_associate err\n",__func__);
...
static void ieee80211_work_empty_start_pendding(struct ieee80211_local *local)
{
	...
	if(local->roc_pendding&&local->roc_pendding_sdata){
		...
		if(roc_timeout || list_empty(&local->work_list)){
			...
			if(roc_timeout){
				atbm_printk_err("%s:pennding roc timeout init_duration(%d) \n",__func__,init_duration);
				ret = -1;
			}
				...
				if(roc_duration>70)
					...
					atbm_printk_err("%s:pennding roc err roc_duration(%d)\n",__func__,roc_duration);
					ret = -1;
				}
			}else{
				BUG_ON(1);
	...
	//				ieee80211_ready_on_channel(&local->hw);
			...
			{
				atbm_printk_err("%s:pennding roc ok  roc_duration (%d)\n",__func__,roc_duration);
				atbm_kfree(roc);
			}
		}
