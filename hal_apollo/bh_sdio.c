...
void atbm_sdio_bh_halt(struct atbm_common *hw_priv)
{
	atbm_printk_err("%s:bh halt\n",__func__);
...
static int atbm_rx_directly(struct atbm_common *hw_priv,struct sk_buff **skb_rx)
{
...
static void atbm_sdio_submit_skb(struct atbm_common *hw_priv,struct sk_buff *skb)
{
...
rx_continue:
	...
	if(directly == true){
		...
		if(unlikely(atbm_rx_directly(hw_priv,&skb_rx) != 0)){
			atbm_sdio_bh_halt(hw_priv);
			goto err;
		}
	}else {
		atbm_sdio_submit_skb(hw_priv, skb_rx);
	}
	...
	while(enough == false){
		...
		if(enough == false){
			...
			if(loop>=MAX_LOOP_POLL_CNT)
				break;
			if((loop >= 3)&&(print == 0)){			
				atbm_printk_always("%s:n_xmits(%d),hw_xmited(%d),need(%d)\n",__func__,
...
#ifndef CONFIG_ATBM_SUPPORT_SG
			break;
#endif
		}
	}while(1);

	if(atbm_sdio_wait_enough_space(hw_priv,txMutiFrameCount) == false){
		atbm_printk_err("%s: wait space timeout\n",__func__);
		goto xmit_err;
	}
#ifdef CONFIG_ATBM_SUPPORT_SG
	data = hw_priv->xmit_buff;
#endif
	spin_lock_bh(&hw_priv->tx_com_lock);
	hw_priv->buf_id_offset = txMutiFrameCount;
	atomic_add(1, &hw_priv->bh_tx);
	hw_priv->n_xmits += txMutiFrameCount;
	hw_priv->hw_bufs_free -= txMutiFrameCount;
	BUG_ON(hw_priv->hw_bufs_free < 0);
	spin_unlock_bh(&hw_priv->tx_com_lock);
	
	if (WARN_ON(atbm_data_write(hw_priv,data, putLen))) {		
		atbm_printk_err("%s: xmit data err\n",__func__);
		goto xmit_err;
	}
#ifndef CONFIG_ATBM_SUPPORT_SG
	if (vif_selected != -1)
		atbm_sdio_free_tx_wsm(hw_priv,(struct wsm_tx *)data);
#endif
	goto xmit_continue;
...
void atbm_sdio_rx_bh(struct atbm_common *hw_priv)
{
	...
	restart:
		...
		hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
		atbm_sdio_process_read_data(hw_priv,atbm_bh_read_ctrl_reg_unlock,atbm_data_read_unlock,true);
		hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
...
void atbm_irq_handler(struct atbm_common *hw_priv)
{
	...
	if(!in_interrupt())	{
		...
		*/
		...
		hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
#endif
		atbm_sdio_process_read_data(hw_priv,atbm_bh_read_ctrl_reg_unlock,atbm_data_read_unlock,false);
		if (atbm_bh_is_term(hw_priv) || atomic_read(&hw_priv->bh_term)){
			hw_priv->bh_error = 1;
			wake_up(&hw_priv->bh_wq);
		}
	...
	while (1) {
		...
		if (!status && __ALL_HW_BUFS_USED)
		{
			...
			if(ctrl_reg & ATBM_HIFREG_CONT_NEXT_LEN_MASK)
			{
				...
				if(ret){
					atbm_printk_err("0xac8029c Error\n");
				}
				atbm_printk_err("MISS 1\n");

				
				hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
				atbm_sdio_process_read_data(hw_priv,atbm_bh_read_ctrl_reg_unlock,atbm_data_read_unlock,hw_priv->hard_irq == true);
				hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
			...
			rx = 1;
			atbm_printk_err("[bh] next wsm_rx_seq %d wsm_tx_seq %d\n",hw_priv->wsm_rx_seq,hw_priv->wsm_tx_seq);
			atbm_printk_err("[bh] wsm_hiftx_cmd_num %d wsm_hif_cmd_conf_num %d\n",hw_priv->wsm_hiftx_num,hw_priv->wsm_hifconfirm_num);
			atbm_printk_err("[bh] wsm_txframe_num %d wsm_txconfirm_num %d\n",hw_priv->wsm_txframe_num,hw_priv->wsm_txconfirm_num);
			atbm_printk_err("[bh] num_pending[0]=%d num_pending[1]=%d pending[2]=%d pending[3]=%d\n",
